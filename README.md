...
학생 공통 푸터의 `문의하기` 링크는 공개 빌드 설정 `VITE_OPEN_CHAT_URL`로
재정의할 수 있습니다. 설정이 없으면 확인된 카카오 오픈채팅 URL을 기본으로
사용하며, 빈 값 또는 잘못된 URL을 명시하면 비활성 텍스트만 표시합니다. 이 값은
브라우저에 공개되므로 비밀이나 자격 증명을 넣지 않습니다.
