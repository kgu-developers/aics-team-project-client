...
export const adminPresentationEvaluationHandlers = [
  ...
  http.patch(
    ...
    async ({ params, request }) => {
      if (request.headers.get('authorization') !== `Bearer ${demoAdminAccessToken}`) {
        return HttpResponse.json({ message: '관리자 로그인이 필요합니다.' }, { status: 401 });
      }
      if (params.sectionId !== adminPresentationEvaluationsFixture.section.id) {
        return HttpResponse.json({ message: '담당 분반만 수정할 수 있습니다.' }, { status: 403 });
      }
      ...
      if (!body.teams || !body.startsAt || !body.endsAt) {
        return HttpResponse.json({ message: '필수 설정이 없습니다.' }, { status: 400 });
      }
      const teamOrders = new Map(body.teams.map(team => [team.teamId, team.presentationOrder]));
      adminPresentationEvaluationsFixture.evaluationPeriod = {
        startsAt: body.startsAt,
        endsAt: body.endsAt,
      };
      adminPresentationEvaluationsFixture.teams = adminPresentationEvaluationsFixture.teams.map(team => ({
        ...team,
        presentationOrder: teamOrders.get(team.teamId) ?? team.presentationOrder,
      }));
