...
it('제안서 상위 구성 영역에 순서 번호를 표시한다', async () => {
  const project = createProjectProposalFixture();
  server.use(
  ...
  try {
    await screen.findByRole('heading', { name: project.title });
    expect(
      screen.getByRole('heading', { level: 3, name: '1. 주제' }),
    ).toBeVisible();
    expect(
      screen.getByRole('heading', { level: 3, name: '2. 데이터 구성' }),
    ).toBeVisible();
    expect(
      screen.getByRole('heading', { level: 3, name: '3. 화면 구성' }),
    ).toBeVisible();
    expect(
      screen.getByRole('heading', { level: 3, name: '4. 팀 운영 방식' }),
    ).toBeVisible();
  } finally {
    cleanup();
  }
