...
**“관리자·학생 통합 E2E 18단계를 검증해줘”**라고 요청한다. 관리자 화면에서 강좌·분반과 학생 7명·팀 2개를 만들고, 전원 주제 투표부터 문서 제출·피드백·발표 평가·최종 승인까지 검증한다. 데이터 생성과 정리를 위한 직접 API 호출 없이 브라우저 UI만 사용한다. 실패 단계와 선행 실패로 건너뛴 단계를 따로 기록하며 독립된 흐름은 계속 검증한다.

[통합 실행·로그인 계정·수동 재생성 가이드](./docs/integrated.md)에 환경변수, 계정별 순서, 실행 데이터셋과 초기화 절차를 정리했다. 통합 suite는 관리자 계정이 필요하며 실행마다 새 데이터가 남는다. 아래의 기존 `test:e2e`/`test:e2e:live` 명령은 준비된 학생 계정으로 개별 회귀 시나리오를 실행한다.
