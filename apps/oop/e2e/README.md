...
`pnpm test:e2e:integrated`는 관리자 화면에서 강좌·분반과 학생 7명·팀 2개를 만들고, 전원 주제 투표부터 문서 제출·피드백·발표 평가·최종 승인까지 18단계를 검증한다. 데이터 생성과 정리를 위한 직접 API 호출 없이 브라우저 UI만 사용한다. 실패 단계와 선행 실패로 건너뛴 단계를 따로 기록하며 독립된 흐름은 계속 검증한다.
